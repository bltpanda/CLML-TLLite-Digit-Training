...
class ClassificationResultsAdapter :
    ...
    private var categories: MutableList<Float?> = mutableListOf()

    @SuppressLint("NotifyDataSetChanged")
    fun updateResults(listClassifications: Array<FloatArray>?) {

        categories.clear()
        // 将listClassifications中的元素添加到categories中
        listClassifications?.first()?.let {
            categories.addAll(it.asList())
        }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        categories[position]?.let { float ->
            holder.bind(position.toString(), float)
        }
    }

...
class DigitClassifier {
  ...
  open var interpreter: Interpreter
  open var inputImageWidth: Int
  open var inputImageHeight: Int
  ...
  func classify(image: UIImage, completion: @escaping ((Result<String>) -> ())) {
    DispatchQueue.global(qos: .background).async {
      ...
      do {
        ...
        try self.interpreter.invoke()
          ...
          print("-----debug \(self.interpreter.signatureKeys)")
      ...
      let maxIndex = results.firstIndex(of: maxConfidence) ?? -1
        let humanReadableResult = "Predicted: \(String(describing: transToLabel(from: maxIndex)))\nConfidence: \(maxConfidence)"
    ...
    func train(image: UIImage, labelIndex: Int, completion: @escaping ((Result<String>) -> ())) {
      DispatchQueue.global(qos: .background).async {
        let outputTensor: Tensor
        do {
          // Preprocessing: Convert the input UIImage to (28 x 28) grayscale image to feed to TF Lite model.
          ...
          // Copy the RGB data to the input `Tensor`.
            let runner = try self.interpreter.signatureRunner(with: "train")
            var yLabel: Array<Float32> = Array(repeating: 0.0, count: 11)
        
            yLabel[labelIndex] = 1.0
            let floatPtr = UnsafeBufferPointer(start: yLabel, count: yLabel.count)

            // 将 `floatArray` 转换为 `Data`
            let yData = Data(buffer: floatPtr)
            
            try runner.invoke(with: ["x":rgbData, "y":yData])

            outputTensor = try runner.output(named: "loss")
        } catch let error {
          print("Failed to invoke the interpreter with error: \(error.localizedDescription)")
          DispatchQueue.main.async {
            completion(.error(ClassificationError.internalError(error)))
        ...
        let results = outputTensor.data.toArray(type: Float32.self)
        let loss = results.first ?? -1
        let humanReadableResult = "Finished, loss: \(loss)"
